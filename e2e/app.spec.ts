...
test("onboards both identities with isolated cookies and an invite", async ({ browser, baseURL }) => {
  ...
  await expect(chelseaPage.context().cookies()).resolves.toEqual(expect.arrayContaining([expect.objectContaining({ name: authCookieName })]));
  const adamCookies = await adam.cookies();
  const chelseaCookies = await chelsea.cookies();
  expect(adamCookies.find((cookie) => cookie.name === authCookieName)?.value).not.toBe(chelseaCookies.find((cookie) => cookie.name === authCookieName)?.value);
...
test("keeps owner-private ledger controls out of the other member's detail view", async ({ browser, baseURL }) => {
  ...
  try {
    ...
    const sourceUpdate = await adamPage.request.put(`${fakeOrigin}/v1/plans/fake-plan-adam/transactions/fake-transaction-adam-1`, {
