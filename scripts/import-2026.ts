...
function openReadOnlyDatabase(filename: string): Database.Database {
  ...
  try {
    ...
    if (row.version !== CURRENT_SCHEMA_VERSION)
      throw new Error(`SQLite schema version ${row.version ?? 0} is not current`);
    return db;
  } catch (error) {
    db.close();
    throw error;
...
if (!environment) {
  throw new Error(
    "usage: pnpm import:2026 -- --environment|--env <dev|development|prod|production> --transactions <file> --split-view <file> --household <id> [--apply]",
  );
}
...
if (!transactionsPath || !splitViewPath || !householdId)
  throw new Error(
    "usage: pnpm import:2026 -- --environment|--env <dev|development|prod|production> --transactions <file> --split-view <file> --household <id> [--apply]",
  );
...
if (report.errors.length > 0) {
  console.error(
    JSON.stringify(
      { mode: apply ? "apply" : "dry-run", environment, parsed, validationErrors: report.errors, writes: "none" },
      null,
      2,
    ),
  );
  ...
  if (!databasePath)
    throw new Error(
      `No DATABASE_PATH configured for ${environment}; set it in the process environment or matching env file`,
    );
        ...
        JSON.stringify(
          {
            ...
            validationErrors: [],
            preflight: {
              insert: result.preflight.insert,
              exactSkip: result.preflight.skip,
              immutableConflict: result.preflight.conflict,
            },
            applyOutcome: {
              inserted: result.applied,
              exactSkip: result.skipped,
              immutableConflict: result.conflicts,
