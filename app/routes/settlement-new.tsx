...
export function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    ...
    return secureData({
      user,
      earliest: entries[0]?.date ?? new Date().toISOString().slice(0, 10),
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    if (form.get("confirmPayment") !== "on")
      return secureData({ error: "Confirm that the real payment occurred before creating a settlement." });
    const start = String(form.get("startDate") ?? "").trim();
    const end = String(form.get("endDate") ?? "").trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(start) || !/^\d{4}-\d{2}-\d{2}$/.test(end) || start > end)
      return secureData({ error: "Choose a valid inclusive date range." });
    ...
    db.transaction(() => {
      db.prepare(
        ...
        preview.netMinor === 0 ? null : debtor,
        preview.netMinor === 0 ? null : creditor,
        Math.abs(preview.netMinor),
      );
    ...
    return secureData({ error: error instanceof Error ? error.message : "Settlement could not be created" });
  } finally {
    db.close();
  }
}
...
export default function SettlementNew({ loaderData, actionData }: Route.ComponentProps) {
  const result =
    actionData && typeof actionData === "object" ? (actionData as { error?: unknown; settlementId?: unknown }) : null;
  ...
  return (
    <section className="max-w-3xl">
      <h1 className="text-3xl font-semibold">Settle up</h1>
      <p className="mt-2 text-slate-600">Select an inclusive date range. Every entry can be settled only once.</p>
      <Form method="post" className="mt-6 space-y-4 rounded border bg-white p-4">
        <label>
          ...
          <input
            ...
            type="date"
            name="startDate"
            defaultValue={loaderData.earliest}
            required
          />
        </label>
        <label>
        ...
        <label className="flex items-center gap-2">
          <input type="checkbox" name="confirmPayment" /> I confirm the real payment occurred.
        </label>
      ...
      {settlementId ? (
        <>
          <ActionFeedback status="Settlement created." focusKey={actionData} />
          <Link ref={settlementLinkRef} className="mt-4 inline-block underline" to={`/settlements/${settlementId}`}>
            Open settlement
          </Link>
        </>
      ) : (
        <Link className="mt-4 inline-block underline" to="/ledger">
          Back to ledger
        </Link>
      )}
    </section>
