...
import {
  ...
  retrySourcePosting,
  verifyManualTask,
} from "~/services/inbox-orchestration.server";
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  try {
    ...
    const rows = db
      ...
      .all(entry.id) as Array<{ member_key: "adam" | "chelsea"; amount_minor: number }>;
    const shares: { adam: number; chelsea: number } = { adam: 0, chelsea: 0 };
    for (const row of rows) shares[row.member_key] = row.amount_minor;
    if (
      rows.length !== 2 ||
      new Set(rows.map((row) => row.member_key)).size !== 2 ||
      rows.some((row) => !Number.isSafeInteger(row.amount_minor)) ||
      shares.adam + shares.chelsea !== entry.amount_minor
    ) {
    ...
    `,
      ...
      .get(user.id, entry.id) as
      { id: string; status: string; intended_target_json: string; last_error: string | null } | undefined;
    const posting = db
      .prepare(
        `
    ...
    const settings = db
      .prepare("select currency_iso_code, currency_decimal_digits from plan_settings where user_id = ?")
      .get(user.id) as { currency_iso_code: string; currency_decimal_digits: number } | undefined;
    let ownerTask: { id: string; status: string; intendedTarget: unknown; errorCode?: string } | null = null;
    if (task) {
      let intendedTarget: unknown = null;
      try {
        intendedTarget = JSON.parse(task.intended_target_json);
      } catch {
        /* stable owner-visible error below */
      }
      ownerTask = {
        id: task.id,
        status: task.status,
        intendedTarget,
        ...(task.last_error ? { errorCode: "manual-task-needs-attention" } : {}),
      };
    }
    return secureData({
      entry: {
      ...
      currency: settings
        ? ({
            isoCode: settings.currency_iso_code,
            decimalDigits: settings.currency_decimal_digits,
      ...
      ownerPrivate: {
        posting: posting
          ? {
              ...
              ...(posting.last_error ? { errorCode: "source-posting-needs-attention" } : {}),
            }
          : null,
        task: ownerTask,
      },
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    if (intent === "retry-source") {
      ...
      return secureData(
        result.posted
          ? { posted: true, status: result.status }
          : { error: result.error ?? "Source retry failed.", status: result.status },
      );
    }
    ...
    if (intent === "save-manual") {
      const settings = db.prepare("select splitting_category_id from plan_settings where user_id = ?").get(user.id) as
        { splitting_category_id: string | null } | undefined;
      prepareManualTask(
        db,
        user.id,
        taskId,
        settings?.splitting_category_id ?? null,
        form.getAll("allocationCategoryId").map(String),
        form.getAll("allocationAmountMinor").map((value) => Number(value)),
      );
      return secureData({ prepared: true });
    }
    if (intent === "verify") {
      const settings = db
        .prepare("select currency_decimal_digits from plan_settings where user_id = ?")
        .get(user.id) as { currency_decimal_digits: number } | undefined;
...
function targetLines(target: unknown): Array<{ categoryId: string; amountMinor: number }> {
  ...
  return target.lines.filter(
    (line): line is { categoryId: string; amountMinor: number } =>
      typeof line === "object" &&
      line !== null &&
      "categoryId" in line &&
...
export default function LedgerEntry({ loaderData, actionData }: Route.ComponentProps) {
  ...
  const actionStatus =
    actionData && "posted" in actionData && actionData.posted
      ...
      : actionData && "prepared" in actionData && actionData.prepared
        ? "Manual target prepared."
        : actionData && "verified" in actionData && actionData.verified
          ? "Manual split verified."
          : actionData && "dismissed" in actionData && actionData.dismissed
            ? "Manual task dismissed."
            : null;
  return (
    <section className="max-w-2xl">
      <Link className="text-sm underline" to="/ledger">
        Back to ledger
      </Link>
      <h1 className="mt-3 text-3xl font-semibold">{loaderData.entry.description}</h1>
      <p className="mt-2 text-slate-600">
        {loaderData.entry.date} · {displayAmount(loaderData.entry.amountMinor, loaderData.currency)} · paid by{" "}
        {loaderData.entry.payerMemberKey}
      </p>
      <div className="mt-6 rounded border bg-white p-4">
        <h2 className="font-semibold">Shares</h2>
        <ul className="mt-2 list-disc pl-5">
          <li>adam: {displayAmount(loaderData.entry.shares.adam, loaderData.currency)}</li>
          <li>chelsea: {displayAmount(loaderData.entry.shares.chelsea, loaderData.currency)}</li>
        </ul>
      </div>
      {posting && (
        <div className="mt-6 rounded border bg-white p-4">
          <h2 className="font-semibold">YNAB source update</h2>
          <p>
            {posting.status}
            {posting.errorCode ? ": Source posting needs attention." : ""}
          </p>
          {(posting.status === "failed" || posting.status === "conflict") && (
            <Form method="post" className="mt-3">
              <input type="hidden" name="postingId" value={posting.id} />
              <Button variant="secondary" name="intent" value="retry-source" type="submit">
                Retry source update
              </Button>
            </Form>
          )}
      ...
      {task && (
        <div className="mt-6 rounded border bg-white p-4">
          <h2 className="font-semibold">Manual YNAB steps</h2>
          <ol className="mt-2 list-decimal space-y-1 pl-5">
            <li>Open the exact YNAB transaction for {loaderData.entry.date} and this payee.</li>
            <li>Edit it; do not delete or recreate it.</li>
            <li>Preserve per-line payees and memos, then set the listed categories and amounts.</li>
            <li>Approve the transaction, return here, and press Verify.</li>
          </ol>
          <Form method="post" className="mt-3 space-y-2">
            <input type="hidden" name="taskId" value={task.id} />
            {manualLines.slice(0, -1).map((line, index) => (
              <div className="grid grid-cols-2 gap-2" key={`${line.categoryId}-${index}`}>
                <input
                  className="rounded border p-2"
                  name="allocationCategoryId"
                  defaultValue={line.categoryId}
                  aria-label={`Owner category ${index + 1}`}
                />
                <input
                  className="rounded border p-2"
                  name="allocationAmountMinor"
                  type="number"
                  defaultValue={line.amountMinor}
                  aria-label={`Owner amount ${index + 1}`}
                />
              </div>
            ))}
            <Button variant="secondary" name="intent" value="save-manual" type="submit">
              Save allocation guidance
            </Button>
          </Form>
          {task.errorCode && (
            <p role="alert" className="mt-3 text-red-700">
              Manual task needs attention.
            </p>
          )}
          <Form method="post" className="mt-4 flex gap-2">
            <input type="hidden" name="taskId" value={task.id} />
            <Button variant="primary" name="intent" value="verify" type="submit">
              Verify
            </Button>
            <Button variant="secondary" name="intent" value="dismiss" type="submit">
              Dismiss
            </Button>
          </Form>
        </div>
