...
interface InboxTransaction extends YnabTransaction { reviewToken: string; }

function settingsFor(db: ReturnType<typeof database>, userId: string) {
  return db.prepare("select plan_id, currency_decimal_digits, splitting_category_id from plan_settings where user_id = ?").get(userId) as { plan_id: string; currency_decimal_digits: number; splitting_category_id: string | null } | undefined;
}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    ...
    if (!settings) return secureData({ user, error: "Select your YNAB plan in settings before reviewing transactions.", transactions: [] as InboxTransaction[] });
    ...
    const transactions = (await gateway.getUnapprovedTransactions(settings.plan_id))
      .filter((transaction) => sourceAccounts.has(transaction.account_id) && !transaction.deleted && !transaction.transfer_account_id && !decisions.has(transaction.id))
      .map((transaction) => ({ ...transaction, reviewToken: reviewToken(getEnv().SESSION_SECRET, user, { planId: settings.plan_id, currencyDecimalDigits: settings.currency_decimal_digits, splittingCategoryId: settings.splitting_category_id }, transaction) }));
    return secureData({ user, error: null, transactions });
  } catch (error) {
    if (error instanceof Response) throw error;
    return secureData({ user: null, error: error instanceof Error ? error.message : "YNAB inbox unavailable", transactions: [] as InboxTransaction[] });
  } finally { db.close(); }
}

function parseSplit(form: FormData) {
  const type = String(form.get("splitType") ?? "equal");
  if (type === "exact") return { type: "exact" as const, otherAmountMinor: Number(form.get("otherAmountMinor") ?? NaN) };
  if (type === "percentage") return { type: "percentage" as const, otherBasisPoints: Number(form.get("otherBasisPoints") ?? NaN) };
...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    if (decision === "not_shared") {
      saveNotSharedDecision(db, user, { planId: settings.plan_id, currencyDecimalDigits: settings.currency_decimal_digits, splittingCategoryId: settings.splitting_category_id }, transaction, token, getEnv().SESSION_SECRET);
      return secureData({ saved: true });
    }
    const result = await saveInboxDecision({
      db,
      user,
      settings: { planId: settings.plan_id, currencyDecimalDigits: settings.currency_decimal_digits, splittingCategoryId: settings.splitting_category_id },
      gateway,
      transaction,
      decision: "shared",
...
export default function Inbox({ loaderData, actionData }: Route.ComponentProps) {
  const result = actionData && typeof actionData === "object" ? actionData as { error?: unknown; saved?: unknown } : null;
  ...
  return <section><h1 className="text-3xl font-semibold">Unapproved transactions</h1><p className="mt-2 text-slate-600">Save the ledger record first. API updates are optional and never used for existing splits.</p><ActionFeedback error={actionError} status={actionStatus} focusKey={actionData} /><div className="mt-6 space-y-4">{loaderData.transactions.length === 0 && <p className="rounded border bg-white p-4">No unapproved transactions require review.</p>}{loaderData.transactions.map((transaction) => <article className="rounded border bg-white p-4" key={transaction.id}><h2 className="font-semibold">{transaction.payee_name ?? "(no payee)"}</h2><p className="text-sm text-slate-600">{transaction.date} · {transaction.account_name ?? transaction.id} · {transaction.amount}</p><Form method="post" className="mt-4 grid gap-3 sm:grid-cols-2"><input type="hidden" name="transactionId" value={transaction.id} /><input type="hidden" name="reviewToken" value={transaction.reviewToken} /><label>Split type<select className="mt-1 w-full rounded border p-2" name="splitType" defaultValue="equal"><option value="equal">Equal</option><option value="percentage">Percentage</option><option value="exact">Exact</option></select></label><label>Other share (minor units)<input className="mt-1 w-full rounded border p-2" name="otherAmountMinor" type="number" /></label><label>Other share (basis points)<input className="mt-1 w-full rounded border p-2" name="otherBasisPoints" type="number" defaultValue="5000" /></label><label>Actual category ID<input className="mt-1 w-full rounded border p-2" name="categoryId" defaultValue={transaction.category_id ?? ""} /></label><label className="flex items-center gap-2"><input name="updateYnab" type="checkbox" defaultChecked={loaderData.user?.memberKey === "adam"} /> Update unsplit YNAB transaction</label><div className="flex gap-2"><Button variant="primary" name="decision" value="shared" type="submit">Save to ledger</Button><Button variant="secondary" name="decision" value="not_shared" type="submit">Not shared</Button></div></Form></article>)}</div></section>;
}
