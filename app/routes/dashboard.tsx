...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    db.prepare(
      "insert into invites (id, household_id, token_hash, expires_at, invited_member_key) values (?, ?, ?, datetime('now', '+24 hours'), ?)",
    ).run(
      randomUUID(),
      user.householdId,
      createHash("sha256").update(token).digest("hex"),
      user.memberKey === "adam" ? "chelsea" : "adam",
    );
...
export default function Dashboard({ loaderData, actionData }: Route.ComponentProps) {
  ...
  const direction =
    loaderData.direction === "owed" ? "you are owed" : loaderData.direction === "owes" ? "you owe" : "settled";
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Link className="rounded bg-slate-900 px-4 py-2 text-white" to="/inbox">
          Review inbox
        </Link>
      </div>
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <article className="rounded border bg-white p-4">
          <p className="text-sm text-slate-500">Open entries</p>
          <p className="text-2xl font-semibold">{loaderData.openCount}</p>
        </article>
        <article className="rounded border bg-white p-4">
          <p className="text-sm text-slate-500">Pending postings</p>
          <p className="text-2xl font-semibold">{loaderData.postingCounts.pending}</p>
        </article>
        <article className="rounded border bg-white p-4">
          <p className="text-sm text-slate-500">Failed/conflicted postings</p>
          <p className="text-2xl font-semibold">
            {loaderData.postingCounts.failed + loaderData.postingCounts.conflict}
          </p>
        </article>
        <article className="rounded border bg-white p-4">
          <p className="text-sm text-slate-500">Manual YNAB tasks</p>
          <p className="text-2xl font-semibold">{loaderData.manualTaskCount}</p>
        </article>
      </div>
      <div className="mt-8 rounded border bg-white p-4">
        <h2 className="font-semibold">Recent activity</h2>
        <ul className="mt-3 divide-y">
          {loaderData.recentActivity.map((entry) => (
            <li className="flex items-center justify-between gap-4 py-2" key={entry.id}>
              <Link className="underline" to={`/ledger/${entry.id}`}>
                {entry.description}
              </Link>
              <span className="text-sm text-slate-600">{displayAmount(entry.amountMinor, loaderData.currency)}</span>
            </li>
          ))}
        </ul>
      </div>
      <div className="mt-8 rounded border bg-white p-4">
        <h2 className="font-semibold">Invite the other member</h2>
        <Form method="post" className="mt-3">
          <Button variant="secondary" name="intent" value="invite" type="submit">
            Create one-use invite
          </Button>
        </Form>
        <ActionFeedback
          status={actionData?.inviteUrl ? "Invitation created. Copy the invite URL for the other member." : null}
          focusKey={actionData}
        />
        {actionData?.inviteUrl && <p className="mt-3 break-all text-sm">Invite URL: {actionData.inviteUrl}</p>}
      </div>
    </section>
  );
}
