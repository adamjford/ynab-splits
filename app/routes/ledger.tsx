...
export default function Ledger({ loaderData }: Route.ComponentProps) {
  return (
    <section>
      <h1 className="text-3xl font-semibold">Shared ledger</h1>
      <p className="mt-2 text-slate-600">
        Both members see the same ledger. YNAB identifiers and manual controls remain private to the owning member.
      </p>
          ...
          <tbody>
            {loaderData.entries.map((entry) => (
              <tr className="border-b last:border-0" key={entry.id}>
                ...
                <td className="p-3">{displayAmount(entry.amountMinor, loaderData.currency)}</td>
                <td className="p-3">{entry.payerMemberKey}</td>
                <td className="p-3">
                  Adam {displayAmount(entry.shares.adam, loaderData.currency)} · Chelsea{" "}
                  {displayAmount(entry.shares.chelsea, loaderData.currency)}
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {loaderData.entries.length === 0 && <p className="p-4">No ledger entries yet.</p>}
      </div>
    </section>
  );
}
