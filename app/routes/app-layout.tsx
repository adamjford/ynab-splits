...
export default function AppLayout({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <header className="border-b bg-white">
        <div className="mx-auto flex w-full max-w-6xl flex-wrap items-center gap-4 p-4">
          ...
          <nav aria-label="Main navigation" className="flex min-w-0 flex-1 flex-wrap items-center gap-2 text-sm">
            ...
            {loaderData.instanceLabel ? (
              <span
                className="px-2 text-slate-500"
                aria-label={`Active development instance: ${loaderData.instanceLabel}`}
              >
                Instance: {loaderData.instanceLabel}
              </span>
            ) : null}
            <QuickNavigation />
      ...
      <main ref={mainRef} id="main-content" tabIndex={-1} className="mx-auto w-full max-w-6xl p-4">
        <Outlet />
      </main>
    </div>
  );
