  ...
  try {
    const transaction = db.transaction(() => {
      ...
      if (!inviteToken) {
        ...
        db.prepare("insert into memberships (household_id, user_id, member_key) values (?, ?, 'adam')").run(householdId, userId);
        return;
      }
      const tokenHash = createHash("sha256").update(inviteToken).digest("hex");
      const invite = db.prepare("select id, household_id, invited_member_key from invites where token_hash = ? and consumed_at is null and expires_at > CURRENT_TIMESTAMP").get(tokenHash) as { id: string; household_id: string; invited_member_key: "adam" | "chelsea" } | undefined;
      if (!invite) throw new Error("invite is expired or invalid");
      if (db.prepare("select count(*) as count from memberships where household_id = ?").get(invite.household_id) as { count: number } && (db.prepare("select count(*) as count from memberships where household_id = ?").get(invite.household_id) as { count: number }).count >= 2) throw new Error("household already has two members");
      db.prepare("insert into memberships (household_id, user_id, member_key) values (?, ?, ?)").run(invite.household_id, userId, invite.invited_member_key);
