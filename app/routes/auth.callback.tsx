...
export async function loader({ request }: Route.LoaderArgs) {
  const [
    ...
    { database },
    { getEnv },
    { clearOAuthCookie, createAuthCookie, readOAuthCookie },
    { HttpYnabGateway },
  ] = await Promise.all([
    import("~/services/auth.server"),
    import("~/services/request.server"),
    import("~/services/env.server"),
    import("~/services/session.server"),
    import("~/services/ynab.server"),
  ]);
  const env = getEnv();
  const url = new URL(request.url);
  const oauth = readOAuthCookie(request.headers.get("Cookie"), env);
  if (!oauth || oauth.state !== url.searchParams.get("state"))
    throw secureResponse(new Response("OAuth state expired or invalid", { status: 400 }));
  ...
  const gateway = new HttpYnabGateway(
    token.access_token,
    token.refresh_token,
  ...
  try {
    ...
    return secureRedirect(`/onboarding${oauth.inviteId ? `?invite=${encodeURIComponent(oauth.inviteId)}` : ""}`, {
      headers,
    });
  } finally {
    db.close();
