...
export function validatePlanSelections(input: PlanSettingsInput, catalog: PlanSelectionCatalog): void {
  if (input.planId !== "default" && !catalog.planIds.has(input.planId)) throw new Error("selected YNAB plan was not found");
  const accountIds = [input.settlementAccountId, ...(input.sourceAccountIds ?? [])].filter((id): id is string => Boolean(id?.trim())).map((id) => id.trim());
  if (accountIds.some((id) => !catalog.accountIds.has(id))) throw new Error("selected account does not belong to the selected YNAB plan");
  if (input.splittingCategoryId?.trim() && !catalog.categoryIds.has(input.splittingCategoryId.trim())) throw new Error("selected splitting category does not belong to the selected YNAB plan");
  if ((input.categoryAssignments ?? []).some((assignment) => !catalog.categoryIds.has(assignment.destinationCategoryId.trim()))) {
...
function saveAssignments(db: AppDatabase, userId: string, assignments: CategoryAssignmentInput[]): void {
  ...
  for (const item of assignments) {
    ...
    if (!sourceId || !destinationId || seen.has(sourceId)) throw new Error("category mappings must have unique, nonempty IDs");
    seen.add(sourceId);
...
export function savePlanSettings(db: AppDatabase, userId: string, input: PlanSettingsInput): void {
  ...
  db.transaction(() => {
    ...
    if (planChanged && unresolvedPosting(db, userId)) throw new Error("cannot change plan while a YNAB posting is unresolved");
    ...
    `).run(userId, input.planId, input.currencyIsoCode, input.currencyDecimalDigits,
    ...
    if (!planChanged && input.sourceAccountIds) {
      ...
      for (const accountId of new Set(input.sourceAccountIds.map((id) => id.trim()).filter(Boolean))) insert.run(userId, accountId);
    }
