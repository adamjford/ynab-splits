...
import {
  ...
  sourceSnapshotHash,
  verifyCreatedPosting,
  verifyReviewedSnapshotToken,
  verifyReviewedSource,
  verifySourceUpdate,
} from "./ynab-verification.server";
...
describe("YNAB source verification", () => {
  ...
  it("distinguishes parent and target read-back fields", () => {
    ...
    const updated: ReviewedSource = {
      ...source,
      category_id: null,
      approved: true,
      subtransactions: target.subtransactions,
    };
    ...
    const cases: Array<{ remote: typeof updated; expected: RegExp }> = [
      ...
      {
        remote: { ...updated, subtransactions: [{ amount: -18890, category_id: "other" }] },
  ...
  it("checks import id and exact settlement read-back", () => {
    const target: CreatedPostingTarget = {
      import_id: "YS:abc",
      account_id: "a1",
      date: "2026-01-02",
      amount: 3190,
      ...
      approved: true,
      subtransactions: [
        { amount: -6250, category_id: "hospital", memo: "YS:e1" },
        { amount: 9440, category_id: "splitting", memo: "YS:aggregate" },
      ],
    };
    ...
    const mismatches: Array<{ remote: typeof target; expected: RegExp }> = [
      ...
      { remote: { ...target, approved: false }, expected: /approved/ },
      {
        remote: {
          ...target,
          subtransactions: [{ ...target.subtransactions[0], amount: -6251 }, target.subtransactions[1]],
        },
        expected: /subtransactions/,
      },
    ];
  ...
  it("binds signed review tokens to user, plan, transaction, and expiry", () => {
    ...
    expect(
      verifyReviewedSnapshotToken("test-secret", token, { userId: "u1", planId: "p1", transactionId: "t1" }).snapshot,
    ).toEqual(source);
    expect(() =>
      verifyReviewedSnapshotToken("test-secret", token, { userId: "u2", planId: "p1", transactionId: "t1" }),
    ).toThrow(/invalid|expired|mismatch/i);
    expect(() =>
      verifyReviewedSnapshotToken("wrong", token, { userId: "u1", planId: "p1", transactionId: "t1" }),
    ).toThrow(/invalid/i);
    ...
    expect(() =>
      verifyReviewedSnapshotToken(
        "test-secret",
        expired,
        { userId: "u1", planId: "p1", transactionId: "t1" },
