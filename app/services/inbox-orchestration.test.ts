...
import {
  ...
  verifyManualTask,
  voidLedgerEntry,
} from "./inbox-orchestration.server";
...
function setup(): AppDatabase {
  ...
  db.exec(
    "insert into users (id, ynab_user_id, display_name) values ('u1', 'y1', 'Adam'), ('u2', 'y2', 'Chelsea'); insert into households (id, name) values ('h1', 'Home'); insert into memberships (household_id, user_id, member_key) values ('h1', 'u1', 'adam'), ('h1', 'u2', 'chelsea');",
  );
  return db;
}
function insertDecision(db: AppDatabase, id = "d1", source: YnabTransaction = transaction): void {
  db.prepare(
    "insert into ynab_transaction_decisions (id, user_id, plan_id, ynab_transaction_id, decision, source_snapshot_hash, source_snapshot_json) values (?, 'u1', 'p1', ?, 'shared', 'hash', ?)",
  ).run(id, source.id, JSON.stringify(source));
}
...
function insertPosting(db: AppDatabase, id = "p1", status = "pending", source: YnabTransaction = transaction): void {
  ...
  db.prepare(
    "insert into ynab_postings (id, decision_id, user_id, posting_kind, status, import_id, intended_target_json) values (?, 'd1', 'u1', 'source', ?, ?, ?)",
  ).run(
    id,
    status,
    `import-${id}`,
...
const splitTransaction: YnabTransaction = {
  ...transaction,
  subtransactions: [
...
describe("inbox review orchestration", () => {
    ...
    try {
      ...
      await expect(
        saveInboxDecision({
          ...
          settings,
          gateway,
          transaction: { ...transaction, payee_name: "Changed" },
          decision: "not_shared",
          split: { type: "equal" },
          updateYnab: false,
          categoryId: null,
      ...
      await expect(
        saveInboxDecision({
          db,
          user: { ...user, id: "u2", memberKey: "chelsea" },
          settings,
          gateway,
          transaction,
          decision: "not_shared",
      ...
      ).rejects.toThrow(/invalid|expired|mismatch/i);
      await expect(
        saveInboxDecision({
          db,
          user,
          settings: { ...settings, planId: "p2" },
          gateway,
          transaction,
          decision: "not_shared",
          split: { type: "equal" },
      ...
      ).rejects.toThrow(/expired|invalid|mismatch/i);
      db.close();
    } finally {
      vi.useRealTimers();
    }
  ...
  it("commits local state before a remote timeout and classifies it as pending", async () => {
    ...
    expect(db.prepare("select status, last_error from ynab_postings").get()).toMatchObject({
      status: "pending",
      last_error: expect.stringMatching(/pending|retry/i),
    });
  ...
  it("rejects missing review proof and ineligible deleted or transfer sources", async () => {
    ...
    await expect(
      saveInboxDecision({
        ...
        settings,
        gateway,
        transaction: { ...transaction, deleted: true },
        decision: "not_shared",
        split: { type: "equal" },
        updateYnab: false,
        categoryId: null,
    ...
    ).rejects.toThrow(/deleted|transfer/i);
    await expect(
      saveInboxDecision({
        db,
        user,
        settings,
        gateway,
        transaction: { ...transaction, transfer_account_id: "transfer" },
        decision: "not_shared",
        split: { type: "equal" },
        updateYnab: false,
        categoryId: null,
  ...
  it("commits shared ledger state without remote work when updating is disabled", async () => {
    ...
    const result = await saveInboxDecision({
      ...
      transaction,
      decision: "shared",
      split: { type: "exact", otherAmountMinor: 944 },
      updateYnab: false,
      categoryId: null,
      reviewToken: token,
      reviewSecret: "review-secret",
    ...
    expect(db.prepare("select amount_minor from ledger_shares where member_key = 'chelsea'").get()).toEqual({
      amount_minor: 944,
    });
    db.close();
  });
  it("rejects malformed source dates and blank payees before writing a ledger parent", async () => {
    ...
    for (const invalidTransaction of invalidTransactions) {
      ...
      ).rejects.toThrow(/date and description/i);
  ...
  it("rejects malformed corrective dates and blank descriptions before replacement insert", () => {
    for (const invalidInput of [
      { date: "2026-02-30", description: "corrected" },
      { date: "2026-01-02", description: " " },
    ]) {
      const db = setup();
      db.exec(
        "insert into ledger_entries (id, household_id, kind, amount_minor, cash_member_key, entry_date, description) values ('e1', 'h1', 'expense', 100, 'adam', '2026-01-01', 'x'); insert into ledger_shares values ('e1', 'adam', 50), ('e1', 'chelsea', 50);",
      );
      expect(() =>
        correctLedgerEntry(db, "h1", "e1", {
          amountMinor: 100,
          cashMemberKey: "adam",
          kind: "expense",
          ...invalidInput,
          shares: [
            { memberKey: "adam", amountMinor: 50 },
            { memberKey: "chelsea", amountMinor: 50 },
          ],
        }),
      ).toThrow(/date and description/i);
      expect(db.prepare("select count(*) as count from ledger_entries").get()).toEqual({ count: 1 });
  ...
  it("allows only transactional edits before voiding and blocks edits afterward", () => {
    ...
    expect(db.prepare("select description from ledger_entries where id = 'e1'").get()).toEqual({
      description: "edited",
    });
    voidLedgerEntry(db, "e1", "h1");
    expect(db.prepare("select voided_at from ledger_entries where id = 'e1'").get()).toMatchObject({
  ...
  it("creates a linked corrective entry and voids the original atomically", () => {
    ...
    const replacement = correctLedgerEntry(db, "h1", "e1", {
      amountMinor: 120,
      cashMemberKey: "adam",
      kind: "expense",
      date: "2026-01-02",
      ...
      shares: [
        { memberKey: "adam", amountMinor: 60 },
        { memberKey: "chelsea", amountMinor: 60 },
      ],
    });
    expect(
      db.prepare("select correction_of_id, amount_minor from ledger_entries where id = ?").get(replacement),
    ).toEqual({ correction_of_id: "e1", amount_minor: 120 });
    expect(db.prepare("select voided_at from ledger_entries where id = 'e1'").get()).toMatchObject({
      voided_at: expect.any(String),
    });
    db.close();
  ...
  it("rejects corrective cash-member mismatches and unsafe share sums", () => {
    ...
    const mismatchedShares = [
      { memberKey: "chelsea", amountMinor: 50 },
      { memberKey: "other", amountMinor: 50 },
    ] as unknown as Parameters<typeof correctLedgerEntry>[3]["shares"];
    expect(() =>
      correctLedgerEntry(db, "h1", "e1", {
        amountMinor: 100,
        cashMemberKey: "adam",
        ...
        date: "2026-01-02",
        description: "corrected",
        shares: mismatchedShares,
      }),
    ).toThrow(/member|share/i);
    expect(() =>
      correctLedgerEntry(db, "h1", "e1", {
        ...
        description: "corrected",
        shares: [
          { memberKey: "adam", amountMinor: Number.MAX_SAFE_INTEGER },
          { memberKey: "chelsea", amountMinor: 1 },
        ],
      }),
    ).toThrow(/shares/i);
    expect(db.prepare("select count(*) as count from ledger_entries").get()).toEqual({ count: 1 });
  ...
  it("classifies every source update outcome without losing local state", async () => {
    ...
    const cases: Array<{ name: string; gateway: YnabGateway; expected: string }> = [
      {
        name: "success",
        gateway: { ...gateway, getTransaction: async () => transaction, updateTransaction: async () => updated },
        expected: "succeeded",
      },
      {
      ...
      {
        name: "stale conflict",
        gateway: { ...gateway, getTransaction: async () => ({ ...transaction, payee_name: "Someone else" }) },
        expected: "conflict",
      },
      {
        ...
        gateway: {
          ...gateway,
          getTransaction: async () => transaction,
          updateTransaction: async () => ({ ...updated, approved: false }),
        },
        expected: "conflict",
      },
      {
  ...
  it("rejects existing splits without a configured splitting category", async () => {
    ...
    ).rejects.toThrow(/Splitting category/i);
    db.close();
  });
  ...
  it("verifies, dismisses, and prepares owner-scoped manual tasks", async () => {
    ...
    insertDecision(db);
    db.prepare(
      "insert into manual_ynab_tasks (id, decision_id, status, intended_target_json) values ('m1', 'd1', 'action_needed', ?)",
    ).run(JSON.stringify(target));
    expect(
      verifyManualSplitReadback(
        {
          id: remote.id,
          amountMinor: remote.amount / 10,
          accountId: remote.account_id,
          date: remote.date,
          payeeName: remote.payee_name,
          approved: remote.approved,
          subtransactions: remote.subtransactions.map((line) => ({
            categoryId: line.category_id,
            amountMinor: line.amount / 10,
    ...
    expect(await verifyManualTask(db, "u2", "m1", gateway, 2)).toMatchObject({
      verified: false,
      error: expect.stringMatching(/not actionable|owned/i),
    });
    expect(() => dismissManualTask(db, "u2", "m1")).toThrow(/not found|owned|resolved/i);
    const verification = await verifyManualTask(
      db,
      user.id,
      "m1",
      { ...gateway, getTransaction: async () => remote },
      2,
    );
    ...
    expect(
      JSON.parse(
        (
          db.prepare("select intended_target_json from manual_ynab_tasks where id = 'm1'").get() as {
            intended_target_json: string;
          }
        ).intended_target_json,
      ).lines,
    ).toHaveLength(2);
    db.prepare("update manual_ynab_tasks set intended_target_json = ?, status = 'action_needed' where id = 'm1'").run(
      JSON.stringify({
        parentAmountMinor: -1889,
        accountId: "a1",
        date: "2026-01-01",
        approved: true,
        lines: [
          { categoryId: "groceries", amountMinor: -945 },
          { categoryId: "splitting", amountMinor: -944 },
        ],
      }),
    );
    prepareManualTask(db, user.id, "m1", "splitting", ["groceries"], [-945]);
    expect(await verifyManualTask(db, user.id, "missing", gateway, 2)).toMatchObject({
      verified: false,
      error: expect.stringMatching(/not actionable|owned/i),
    });
    expect(() => prepareManualTask(db, user.id, "missing", "splitting", ["groceries"], [-945])).toThrow(
      /not actionable|owned/i,
    );
  ...
  it("records manual verification mismatches and transport failures", async () => {
    ...
    for (const remote of [{ ...splitTransaction, payee_name: "Wrong" }, null]) {
      ...
      if (remote)
        expect(db.prepare("select status from manual_ynab_tasks where id = 'm1'").get()).toEqual({
          status: "action_needed",
        });
      if (remote)
        expect(db.prepare("select last_error from manual_ynab_tasks where id = 'm1'").get()).toMatchObject({
  ...
  it("retries source postings through terminal, conflict, success, and failure states", async () => {
    ...
    const scenarios: Array<{ status: string; gateway: YnabGateway; expected: string }> = [
      ...
      {
        status: "pending",
        gateway: { ...gateway, getTransaction: async () => ({ ...transaction, payee_name: "changed" }) },
        expected: "conflict",
      },
  ...
  it("rejects unsafe correction and settlement-linked edits", () => {
    ...
    expect(() =>
      correctLedgerEntry(db, "h1", "e1", {
        ...
        description: "bad",
        shares: [
          { memberKey: "adam", amountMinor: 100 },
          { memberKey: "adam", amountMinor: 0 },
        ],
      }),
    ).toThrow(/shares/i);
    db.close();
  ...
  it("covers income, alternate member, fallback payee, and retry error classification", async () => {
    ...
    const result = await saveInboxDecision({
      ...
      gateway: { ...gateway, getTransaction: async () => income, updateTransaction: async () => updated },
      transaction: income,
      decision: "shared",
      split: { type: "equal" },
      updateYnab: true,
    ...
    expect(db.prepare("select kind, description from ledger_entries").get()).toEqual({
      kind: "income",
    ...
    expect(
      (
        await retrySourcePosting(db, user.id, "generic", {
          ...gateway,
          getTransaction: async () => {
            throw new Error("offline");
          },
    ...
    expect(() =>
      saveNotSharedDecision(
        db,
        user.id === "u2" ? otherUser : user,
        incomeSettings,
        { ...income, deleted: true },
        token,
        "review-secret",
      ),
    ).toThrow(/Transfers|deleted/i);
    db.close();
  });
});
