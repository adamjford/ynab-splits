...
export function authenticatedUser(request: Request, db: AppDatabase): { id: string; displayName: string; householdId: string; memberKey: "adam" | "chelsea" } {
  ...
  const row = db.prepare(`select u.id, u.display_name, m.household_id, m.member_key from users u join memberships m on m.user_id = u.id where u.id = ?`).get(userId) as { id: string; display_name: string; household_id: string; member_key: "adam" | "chelsea" } | undefined;
