...
describe("ledger query boundaries", () => {
  it("loads only active household entries and projects safe shared facts", () => {
    ...
    expect(toSharedLedgerEntry(entries[0])).toEqual({ id: "e1", kind: "expense", amountMinor: 100, payerMemberKey: "adam", date: "2026-01-01", description: "visible", shares: { adam: 40, chelsea: 60 } });
    db.close();
  });

  it("reports missing, duplicate, or mismatched shares instead of dropping a parent", () => {
    const db = createDatabase(":memory:");
    db.exec("insert into households (id, name) values ('h1', 'Home'); insert into ledger_entries (id, household_id, kind, amount_minor, cash_member_key, entry_date, description) values ('bad', 'h1', 'expense', 100, 'adam', '2026-01-01', 'bad');");
  ...
  it("rejects unsafe shared projection input", () => {
    expect(() => toSharedLedgerEntry({ id: "bad", kind: "expense", amountMinor: 1, cashMemberId: "adam", shares: [{ memberId: "adam", amountMinor: 1 }], date: "2026-01-01", description: "bad" } as unknown as Parameters<typeof toSharedLedgerEntry>[0])).toThrow(/corruption/i);
  });
