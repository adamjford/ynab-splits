...
describe("manual split targets", () => {
  ...
  it("verifies readback with order-independent category/amount matching", () => {
    ...
    expect(
      verifyManualSplitReadback(
        { ...source, approved: true, subtransactions: [...source.subtransactions].reverse() },
        target,
      ),
    ).toEqual({ matches: true, differences: [] });
    expect(
      verifyManualSplitReadback({ ...source, approved: false, subtransactions: source.subtransactions }, target)
        .matches,
    ).toBe(false);
  });
  it("includes normalized payee and memo in order-independent tuple matching", () => {
    ...
    expect(
      verifyManualSplitReadback(
        {
          ...reordered,
          subtransactions: [{ ...reordered.subtransactions[0], memo: "tampered" }, reordered.subtransactions[1]],
        },
        target,
      ).differences.join(" "),
    ).toMatch(/memo|split lines/i);
  });
  ...
  it("rejects missing allocations, non-integer shares, and mismatched totals", () => {
    ...
    expect(() =>
      buildManualSplitTarget(source, Number.NaN, [{ categoryId: "groceries", amountMinor: -945 }], "splitting"),
    ).toThrow(/integer/i);
    expect(() =>
      buildManualSplitTarget(source, -944, [{ categoryId: "groceries", amountMinor: -945 }], "splitting"),
    ).toThrow(/total/i);
  });
  ...
  it("rejects every non-safe-integer owner allocation amount", () => {
    for (const amountMinor of [
      ...
      expect(() =>
        buildManualSplitTarget(source, -945, [{ categoryId: "groceries", amountMinor }], "splitting"),
  ...
  it("normalizes an absent payee on both source and target", () => {
    ...
    const targetWithoutPayee = buildManualSplitTarget(
      sourceWithoutPayee,
      -945,
      [{ categoryId: "groceries", amountMinor: -945 }],
      "splitting",
    );
    expect(verifyManualSplitReadback({ ...sourceWithoutPayee, approved: true }, targetWithoutPayee)).toEqual({
      matches: true,
      differences: [],
    });
  });
  ...
  it("uses stable none labels when a mismatched payee is absent on either side", () => {
    const target = buildManualSplitTarget(source, -945, [{ categoryId: "groceries", amountMinor: -945 }], "splitting");
    expect(
      verifyManualSplitReadback({ ...source, payeeName: undefined, approved: true }, target).differences,
    ).toContain("payee: expected Amazon, got (none)");
    expect(
      verifyManualSplitReadback({ ...source, approved: true }, { ...target, payeeName: undefined }).differences,
    ).toContain("payee: expected (none), got Amazon");
  });
  it("accepts an omitted parent id when all other fields and lines match", () => {
    const target = buildManualSplitTarget(source, -945, [{ categoryId: "groceries", amountMinor: -945 }], "splitting");
    expect(verifyManualSplitReadback({ ...source, approved: true }, target)).toEqual({
      matches: true,
      differences: [],
    });
  });
