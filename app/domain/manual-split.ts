...
function sortedLines(
  lines: Array<{ categoryId: string | null; amountMinor: number; payeeName?: string | null; memo?: string | null }>,
): string[] {
  return lines.map(lineTuple).sort();
}

export function buildManualSplitTarget(
  source: ManualSourceTransaction,
  memberShareMinor: number,
  ...
  const existingByCategory = new Map(
    source.subtransactions
      .filter((line) => line.categoryId !== splittingCategoryId)
  ...
  return {
    ...
    lines: [
      ...ownerLines,
      {
        ...
        amountMinor: counterpartyAmount,
        payeeName: splittingExisting?.payeeName ?? source.payeeName,
        memo: splittingExisting?.memo ?? null,
      },
    ],
    approved: true,
  };
...
export function verifyManualSplitReadback(
  source: ManualSourceTransaction,
  target: ManualSplitTarget,
): ManualVerification {
  const differences: string[] = [];
  if (target.parentId !== undefined && source.id !== target.parentId)
    differences.push(`parent: expected ${target.parentId}, got ${source.id}`);
  if (source.amountMinor !== target.parentAmountMinor)
    differences.push(`parent amount: expected ${target.parentAmountMinor}, got ${source.amountMinor}`);
  if (source.accountId !== target.accountId)
    differences.push(`account: expected ${target.accountId}, got ${source.accountId}`);
  if (source.date !== target.date) differences.push(`date: expected ${target.date}, got ${source.date}`);
  if (normalizedText(source.payeeName) !== normalizedText(target.payeeName))
    differences.push(`payee: expected ${target.payeeName ?? "(none)"}, got ${source.payeeName ?? "(none)"}`);
  if (source.approved !== target.approved)
    differences.push(`approved: expected ${target.approved}, got ${source.approved}`);
  ...
  if (expected.length !== actual.length || expected.some((line, index) => line !== actual[index]))
    differences.push("split lines differ by category, amount, payee, or memo");
