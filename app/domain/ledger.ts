...
export function allocateShares(
  ...
  if (input.type === "equal") {
    ...
    otherAmount = Math.floor((totalMinor * input.otherBasisPoints) / 10_000);
  } else {
...
export function assertLedgerEntry(entry: LedgerEntry): void {
  ...
  if (entry.shares[0].amountMinor < 0 || entry.shares[1].amountMinor < 0 || entry.shares[0].amountMinor + entry.shares[1].amountMinor !== entry.amountMinor) {
    throw new Error("ledger shares must be non-negative and sum to the entry total");
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(entry.date) || !entry.description.trim()) {
