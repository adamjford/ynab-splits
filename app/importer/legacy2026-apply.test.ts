  ...
  transfers: [
    {
      ...
      recordedNetMinor: -944,
    },
  ],
  periods: [
    {
      entryKeys: ["sheet-2026:2"],
      calculatedNetMinor: -944,
      transfer: {
        sourceRow: 3,
        date: "2026-01-02",
        amountMinor: 944,
...
describe("legacy 2026 atomic apply", () => {
  it("preflights and applies deterministic entries and shares with truthful deltas", () => {
    const db = setup();
    databases.push(db);
    expect(preflightLegacy2026(db, "h1", open)).toMatchObject({
      insert: { entries: 1, shares: 2 },
      skip: { entries: 0 },
    });
    ...
    expect(second).toMatchObject({
      blocked: false,
      applied: { entries: 0, shares: 0 },
      skipped: { entries: 1, shares: 2 },
    });
  ...
  it("applies a closed period and blocks immutable conflicts atomically", () => {
    ...
    expect(dryRun).toMatchObject({
      insert: { entries: 1, shares: 2, settlements: 1, items: 1 },
      skip: { entries: 0, shares: 0, settlements: 0, items: 0 },
      conflict: { entries: 0, shares: 0, settlements: 0, items: 0 },
    });
    ...
    expect(db.prepare("select status, amount_minor from settlements where id = 'legacy-settlement:3'").get()).toEqual({
      status: "closed",
      amount_minor: 944,
    });
    ...
    expect(replay).toMatchObject({
      blocked: false,
      applied: { entries: 0, shares: 0, settlements: 0, items: 0 },
      skipped: { entries: 1, shares: 2, settlements: 1, items: 1 },
    });
    ...
    expect(db.prepare("select description from ledger_entries where id = 'legacy:sheet-2026:2'").get()).toEqual({
      description: "Amazon",
    });
  });
  ...
  it("blocks missing related shares or settlement items without applying", () => {
    const sharesDb = setup();
    databases.push(sharesDb);
    applyLegacy2026(sharesDb, "h1", open);
    sharesDb.exec("drop trigger ledger_share_delete_check");
    sharesDb
      .prepare("delete from ledger_shares where entry_id = 'legacy:sheet-2026:2' and member_key = 'chelsea'")
      .run();
  ...
  it("does not partially apply new rows when an immutable conflict is present", () => {
    ...
    const conflict: LegacyImportReport = {
      ...expanded,
      rows: [{ ...expanded.rows[0], description: "Changed" }, expanded.rows[1]],
    };
    ...
    expect(result.applied).toEqual({ entries: 0, shares: 0, settlements: 0, items: 0 });
    expect(db.prepare("select count(*) as count from ledger_entries").get()).toEqual({ count: 1 });
    expect(db.prepare("select count(*) as count from ledger_entries where id = 'legacy:sheet-2026:4'").get()).toEqual({
      count: 0,
    });
  });

  it("classifies duplicate keys, aliases, settlement mismatches, and competing links", () => {
    const duplicateDb = setup();
    databases.push(duplicateDb);
    ...
    duplicateDb
      .prepare(
        "insert into ledger_entries (id, household_id, kind, amount_minor, cash_member_key, entry_date, description, legacy_key) values ('other', 'h1', 'expense', 1889, 'adam', '2026-01-01', 'Amazon', 'sheet-2026:2')",
      )
      .run();
    expect(preflightLegacy2026(duplicateDb, "h1", open).conflicts.join(" ")).toMatch(/already used/i);

    const settlementDb = setup();
    databases.push(settlementDb);
    ...
    settlementDb
      .prepare(
        "insert into settlements (id, household_id, start_date, end_date, debtor_member_key, creditor_member_key, amount_minor, status, acknowledged_payment_at) values ('other-settlement', 'h1', '2026-01-01', '2026-01-02', 'adam', 'chelsea', 944, 'closed', CURRENT_TIMESTAMP)",
      )
      .run();
    settlementDb
      .prepare(
        "insert into ledger_entries (id, household_id, kind, amount_minor, cash_member_key, entry_date, description, legacy_key) values ('legacy:sheet-2026:4', 'h1', 'income', 500, 'chelsea', '2026-01-04', 'Refund', 'sheet-2026:4')",
      )
      .run();
    settlementDb
      .prepare(
        "insert into settlement_items (settlement_id, ledger_entry_id) values ('other-settlement', 'legacy:sheet-2026:4')",
      )
      .run();
    const linkedReport = {
      ...closed,
      periods: [{ ...closed.periods[0], entryKeys: ["sheet-2026:2", "sheet-2026:4"] }],
    };
  ...
  it("rolls back all inserted rows when a SQL failure occurs mid-apply", () => {
    ...
    db.exec(
      "create trigger fail_second_legacy_entry before insert on ledger_entries when new.legacy_key = 'sheet-2026:3' begin select raise(abort, 'forced importer failure'); end",
    );
  ...
  it("reports CLI dry-run preflight deltas without writes and skips the database on parse errors", () => {
    ...
    const run = (
      ...
      configuredPath: string | null | undefined = path,
      flag = "--environment",
    ) => {
      ...
      if (configuredPath) {
        const filename =
          environment === "production" || environment === "prod" ? ".env.production" : ".env.development";
      ...
      return spawnSync(
        ...
        [
          ...
          join(process.cwd(), "scripts/import-2026.ts"),
          flag,
          environment,
          "--transactions",
          transactionsPath,
