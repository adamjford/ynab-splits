...
describe("parseLegacy2026", () => {
  ...
  it("reports row identity mismatches without producing rows", () => {
    expect(parseLegacy2026(transactions.replace("Amazon", "Wrong"), splitView).errors.join(" ")).toMatch(
      /identity|name/i,
    );
  });
  it("keeps trailing periods open and reports signed transfer mismatches", () => {
    const open = parseLegacy2026(
      transactions.split("\n").slice(0, 2).join("\n"),
      splitView.split("\n").slice(0, 5).join("\n"),
    );
    ...
    expect(report.periods.at(-1)).toMatchObject({
      calculatedNetMinor: 944,
      transfer: expect.objectContaining({ amountMinor: 945 }),
    });
  ...
  it("fails closed on malformed CSV, missing headers, and unsupported members", () => {
    ...
    expect(parseLegacy2026(transactions.replace(",Adam,1,1", ",Taylor,1,1"), splitView).errors.join(" ")).toMatch(
      /payer/i,
    );
  });
  it("reports row-level amount, split alignment, and share errors", () => {
    expect(parseLegacy2026(transactions.replace("18.89,Adam", "not-money,Adam"), splitView).errors.join(" ")).toMatch(
      /invalid amount/i,
    );
    expect(
      parseLegacy2026(
        transactions,
        splitView.replace("18.89,Adam,,,,,,,,9.44", "not-money,Adam,,,,,,,,9.44"),
      ).errors.join(" "),
    ).toMatch(/split view amount/i);
    expect(
      parseLegacy2026(transactions, splitView.replace("9.44,,9.45,9.44", "20.00,,9.45,9.44")).errors.join(" "),
    ).toMatch(/exceeds total/i);
    expect(parseLegacy2026(transactions, splitView.split("\n").slice(0, 4).join("\n")).errors.join(" ")).toMatch(
      /missing Split View row/i,
    );
    expect(
      parseLegacy2026(transactions, splitView.replace("2026-01-01,Amazon", "2026-01-02,Amazon")).errors.join(" "),
    ).toMatch(/identity mismatch/i);
  });
...
2026-04-02,Settle Up,5.00,Adam,,,,,,,,5.00,,5.00,0`;
    ...
    expect(report.errors).toEqual([]);
    expect(
      parseLegacy2026("Date,Name,Amount (negative if income),Paid/received by\n2026-05-01", splitView).errors.join(" "),
