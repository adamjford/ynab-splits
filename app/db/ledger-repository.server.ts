...
function assertValidCalendarDate(value: unknown): asserts value is string {
  if (typeof value !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(value)) throw new Error("ledger date must be a calendar date");
...
function assertValidLedgerInput(input: LedgerInsert): void {
  if (input.cashMemberKey !== "adam" && input.cashMemberKey !== "chelsea") throw new Error("ledger entry requires a known cash member");
  if (!Number.isSafeInteger(input.amountMinor) || input.amountMinor <= 0) throw new Error("ledger amount must be a positive safe integer");
  assertValidCalendarDate(input.date);
  if (typeof input.description !== "string" || input.description.trim() === "") throw new Error("ledger description cannot be blank");
  if (!Array.isArray(input.shares) || input.shares.length !== 2) throw new Error("ledger shares require exactly two members");
  ...
  for (const share of input.shares) {
    ...
    if (!Number.isSafeInteger(amountMinor) || amountMinor < 0) throw new Error("ledger shares must be non-negative safe integers");
