...
describe("ledger repository", () => {
  it("uses a fresh file, enables WAL, and rolls back invalid shares atomically", () => {
    ...
    expect(() =>
      insertLedgerEntry(db, {
        ...
        shares: [
          { memberKey: "adam", amountMinor: 40 },
          { memberKey: "chelsea", amountMinor: 40 },
        ],
      }),
    ).toThrow(/sum/i);
  ...
  it("enforces foreign keys for ledger rows", () => {
    ...
    expect(() =>
      db
        .prepare(
          "insert into ledger_entries (id, household_id, kind, amount_minor, cash_member_key, entry_date, description) values (?, ?, ?, ?, ?, ?, ?)",
        )
        .run("e1", "missing", "expense", 1, "adam", "2026-01-01", "x"),
  ...
  it("rejects duplicate or negative shares before opening a transaction", () => {
    ...
    expect(() =>
      insertLedgerEntry(db, {
        ...input,
        shares: [
          { memberKey: "adam", amountMinor: 5 },
          { memberKey: "adam", amountMinor: 5 },
        ],
      }),
    ).toThrow(/two members/i);
    expect(() =>
      insertLedgerEntry(db, {
        ...input,
        shares: [
          { memberKey: "adam", amountMinor: -1 },
          { memberKey: "chelsea", amountMinor: 11 },
        ],
      }),
    ).toThrow(/negative|sum/i);
    expect(db.prepare("select count(*) as count from ledger_entries").get()).toEqual({ count: 0 });
    expect(db.prepare("select count(*) as count from ledger_shares").get()).toEqual({ count: 0 });
    db.close();
  });
  it("rejects malformed direct inputs before writing a parent row", () => {
    const cases: Array<{ name: string; value: unknown }> = [
      ...
      { name: "malformed share", value: [{ memberKey: "adam", amountMinor: 5 }, null] },
      {
        name: "unknown member",
        value: [
          { memberKey: "eve", amountMinor: 5 },
          { memberKey: "chelsea", amountMinor: 5 },
        ],
      },
      {
        name: "fractional share",
        value: [
          { memberKey: "adam", amountMinor: 1.5 },
          { memberKey: "chelsea", amountMinor: 8.5 },
        ],
      },
      {
        name: "unsafe share",
        value: [
          { memberKey: "adam", amountMinor: Number.MAX_SAFE_INTEGER + 1 },
          { memberKey: "chelsea", amountMinor: 0 },
        ],
      },
      {
        name: "negative share",
      ...
      expect(
        () =>
          insertLedgerEntry(db, {
            ...
            shares: [
              { memberKey: "adam", amountMinor: 4 },
              { memberKey: "chelsea", amountMinor: 6 },
            ],
          } as never),
        testCase.name,
      ).toThrow();
