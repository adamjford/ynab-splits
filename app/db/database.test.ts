...
describe("SQLite persistence", () => {
 ...
 it("keeps only one action-needed manual task per decision while retaining history", () => {
  ...
  const insert = db.prepare("insert into manual_ynab_tasks (id, decision_id, status, intended_target_json) values (?, ?, ?, '{}')");
 ...
 it("enforces posting statuses, import idempotency, and per-owner settlement uniqueness", () => {
  ...
  for (const [index, status] of ["open", "voided", "closed"].entries()) {
   db.prepare("insert into settlements (id, household_id, start_date, end_date, amount_minor, status) values (?, 'h1', '2026-01-01', '2026-01-02', 0, ?)").run(`status-settlement-${index}`, status);
  }
  expect(() => db.prepare("insert into settlements (id, household_id, start_date, end_date, amount_minor, status) values ('bad-settlement-status', 'h1', '2026-01-01', '2026-01-02', 0, 'unknown')").run()).toThrow(/check/i);
  ...
  expect(() => insert.run("settlement-u1-again", "s1", "u1", "failed", "settlement-u1-again-import")).toThrow(/unique/i);
 ...
 it("preserves settlement unlink audit history while allowing active reuse", () => {
  ...
  db.exec(`
    ...
    `);
  expect(() => db.prepare("insert into settlement_items (settlement_id, ledger_entry_id) values ('s2', 'e1')").run()).toThrow(/unique/i);
  db.prepare("update settlement_items set unlinked_at = '2026-01-05' where settlement_id = 's1' and ledger_entry_id = 'e1'").run();
  db.prepare("insert into settlement_items (settlement_id, ledger_entry_id) values ('s2', 'e1')").run();
  expect(db.prepare("select unlinked_at from settlement_items where settlement_id = 's1' and ledger_entry_id = 'e1'").get()).toEqual({ unlinked_at: "2026-01-05" });
  db.close();
 });
